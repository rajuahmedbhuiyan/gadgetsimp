...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4.5",
  {
    variants: {
      ...
      /*
       * One step up across the scale.
       *
       * The old `h-8` default and `size-8` icon were below the 36px that a
       * finger reliably hits and read as cramped beside 40px inputs, which is
       * why call sites kept overriding them with an explicit `h-10`. Raising
       * the scale itself is what stops that being the house habit - anything
       * already pinned to a height is unaffected, since an explicit class
       * still wins.
       */
      size: {
        default:
